...
Node3D* Algorithm::hybridAStar(Node3D& start,
  ...
  // NODE POINTER
  Node3D* nPred;  //父节点-当前节点
  Node3D* nSucc;  //子节点

  // float max = 0.f;

  printf("Enter Astar search\n");
  ...
  while (!O.empty()) {
    ...
    // EXPANSION OF NODE
      ...
      if (*nPred == goal || iterations > Constants::iterations) {
        // DEBUG
        cout << "goal test!" << endl;
        if(iterations > Constants::iterations) cout << "---iter----" << endl;
        return nPred;   //当前节点满足goal范围,返回当前节点
      }
      ...
      // CONTINUE WITH SEARCH
        ...
        if (Constants::dubinsShot && nPred->isInRange(goal)) {
          ...
          if (nSucc != nullptr && *nSucc == goal) {
            //DEBUG
            //std::cout << "------DubinsShot-----" << std::endl;
            std::cout << "------ReedsSheppShot-----" << std::endl;
            return nSucc;
          }
        }

//         printf("nPred: %f, %f\n", nPred->getX(), nPred->getY());
...
//           }
          ...
          if (nSucc->isOnGrid(width, height) && configurationSpace.isTraversable(nSucc)) {
            ...
            if (!nodes3D[iSucc].isClosed() || iPred == iSucc) {
              ...
              // if successor not on open list or found a shorter way to the cell or it has the same index as the predecessor
...
void updateH(Node3D& start, const Node3D& goal, Node2D* nodes2D, float* dubinsLookup, int width, int height, CollisionDetection& configurationSpace, Visualize& visualization) {
  ...
  // if reversing is active use reedsSheppPath
...
Node3D* reedsSheppShot(Node3D& start, const Node3D& goal, CollisionDetection& configurationSpace){
  ...
  ReedsSheppStateSpace *reedsSheppPath = new ReedsSheppStateSpace;
  //DEBUG
  //ReedsSheppStateSpace::ReedsSheppPath rsPath ;
  ...
  // rsPath = reedsSheppPath->reedsShepp(q0, q1);
  // std::cout<< "rs_len:"<<rsPath.length()<<std::endl;
  // std::cout<< "rs_length:"<< reedsSheppPath->distance(q0, q1)<<std::endl;

  vector<vector<double>> finalpath;
  finalpath = reedsSheppPath->xingshensample(q0,q1,Constants::reedsSheppStepSize);
  Node3D* reedsSheppNodes = new Node3D [(int)(finalpath.size())];
  int i =0;
  for(;i<finalpath.size();i++) 
  {
    reedsSheppNodes[i].setX(finalpath[i][0]);
    reedsSheppNodes[i].setY(finalpath[i][1]);
    reedsSheppNodes[i].setT(Helper::normalizeHeadingRad(finalpath[i][2]));
    ...
    if (configurationSpace.isTraversable(&reedsSheppNodes[i])) {
      ...
      // std::cout << "rs shot collided, discarding the path" << "\n";
      // delete all nodes
  ...
  //std::cout << "rs shot connected, returning the path" << "\n";
